...
import { categoriesTypes } from "#/utiles/audio_category";
...
export const createaudio: RequestHandler = async(req:CreateAudioRequest, res) =>{
    ...
    if(!audioFile) return res.json(422).json({error: "Audio file is missing!"})
...
export const updateAdio: RequestHandler = async(req:CreateAudioRequest, res) =>{
    const {title, about, category} = req.body
    const poster = req.files?.poster as formidable.File;
    const ownerId = req.user.id;
