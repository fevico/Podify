...
import { getUsersPreviousHistory } from "#/utiles/helper";
...
import user from "#/models/user";
 ...
 export const getRecommendedByprofile: RequestHandler = async(req, res) =>{
  ...
  if(user){
    // we want to sen by the profile

    // fetch users previos history
  ...
  // otherwise we want to generate generic audios 
 ...
 export const getAutoGeneratedPlaylist: RequestHandler = async(req, res) =>{
  ...
  // // 4 would come from autogenerated 
  const category = await getUsersPreviousHistory(req)
  ...
  const playlist = await Playlist.findOne({owner: req.body.id, title});
 ...
 export const getFollowerProfilePublic: RequestHandler = async(req, res) =>{
  const {limit="20", pageNo ="0"} = req.query as pageinationQuery;
 ...
 export const getPlaylistAudios: RequestHandler = async(req, res) =>{
  ...
  const aggregationLogic = [
      ...
      {$match: {_id: new Types.ObjectId(playlistId), visiblity: {$ne: "private"} }},
      {
        $project: {
          items: {
            $slice: [
