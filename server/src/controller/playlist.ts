...
export const createPlaylist : RequestHandler = async(req: CreatePlaylistRequest, res) => {
    ...
    // while createing playlist there can be request or user just want to create an empty playlist
...
export const removePlaylist : RequestHandler = async(req, res) => {
    ...
    if(resId){   
        if(!isValidObjectId(playlistId)) return res.status(422).json({error: "Invalid Audio ID!"})
